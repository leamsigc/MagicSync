...
INFO=$(curl -s -H "X-Api-Key: $API_TOKEN" http://localhost:3000/api/v1/cli/info)
...
# Run all tests
API_TOKEN=your-key ./test.sh all

# Run specific test
API_TOKEN=your-key ./test.sh info
...
PLATFORMS=$(curl -s -H "X-Api-Key: $API_TOKEN" "$BASE_URL/info")
echo "$PLATFORMS" | jq '.'
