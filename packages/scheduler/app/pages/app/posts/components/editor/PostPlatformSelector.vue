...
import type { SocialMediaComplete, BusinessProfile } from '#layers/BaseDB/db/schema';
...
const businessOptions = computed(() => {
  const options = [{ value: null, label: 'All Businesses' }];
  props.businesses.forEach(business => {
    options.push({ value: business.id, label: business.name });
  });
  return options;
});

const filteredAccounts = computed(() => {
  if (!selectedBusinessId.value) return props.accounts;
  return props.accounts.filter(account => account.businessId === selectedBusinessId.value);
});
...
<template>
  <div class="flex gap-2 py-2 overflow-x-auto overflow-y-visible w-full">
    <div class="flex gap-2 w-full">
      <UPopover v-for="account in filteredAccounts" :key="account.id" mode="hover">
        <UChip
          ...
          <UAvatar :src='account.entityDetail?.details?.picture' :alt="account.accountName"
            @click="handleToggle(account)" v-if="account.entityDetail?.details?.picture" />
      ...
      <div v-if="filteredAccounts.length === 0" class="flex items-center justify-center py-4">
        <p class="text-sm text-zinc-500">No accounts for this business</p>
      </div>
      <div class="flex items-center gap-2 px-1">
        <USelect v-model="selectedBusinessId" :items="businessOptions" placeholder="Filter by business"
