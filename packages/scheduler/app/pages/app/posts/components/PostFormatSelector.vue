...
const isFormatDisabled = (format: PostFormat): boolean => {
    return !availableFormats.value.some(f => f?.value === format);
};
</script>
    ...
    <div class="space-y-2">
        ...
        <div class="flex gap-2">
            <UButton v-for="format in formatOptions" :key="format.value" :icon="format.icon"
                :variant="modelValue === format.value ? 'solid' : 'outline'"
                :disabled="isFormatDisabled(format.value as PostFormat)" size="sm"
                @click="modelValue = format.value as PostFormat">
                {{ format.label }}
            </UButton>
        </div>
