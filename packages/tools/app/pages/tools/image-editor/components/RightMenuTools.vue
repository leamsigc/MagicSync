...
const handleAddGuideline = (orientation: 'horizontal' | 'vertical') => {
  try {
    ...
    toast.add({ title: 'Failed to add guideline', color: 'red', icon: 'lucide:x' });
  }
};
...
const handleRemoveGuideline = (id: string) => {
  try {
    ...
    toast.add({ title: 'Guideline removed', color: 'green', icon: 'lucide:check' });
  } catch (error) {
    toast.add({ title: 'Failed to remove guideline', color: 'red', icon: 'lucide:x' });
  }
};
...
const updatePosition = () => {
  try {
    ...
    toast.add({ title: 'Position updated', color: 'green', icon: 'lucide:check' });
  } catch (error) {
    toast.add({ title: 'Failed to update position', color: 'red', icon: 'lucide:x' });
  }
};
...
const handleVisibilityToggle = () => {
  ...
  if (activeObj) {
    ...
    toast.add({ title: activeObj.visible ? 'Layer visible' : 'Layer hidden', color: 'gray' });
  }
};
...
const handleLockToggle = () => {
  ...
  if (activeObj) {
    ...
    toast.add({ title: isLocked ? 'Layer locked' : 'Layer unlocked', color: 'gray' });
  }
};
    ...
    <div class="space-y-4 pb-4 border-b border-gray-200 dark:border-gray-800">
      ...
      <div class="space-y-2.5 pl-6">
        <div class="flex items-center justify-between">
          <span class="text-xs text-gray-700 dark:text-gray-300">{{ t('tools.ruler.toggleRulers', 'Show Rulers')
          }}</span>
          <UToggle v-model="showRulers" @update:model-value="handleToggleRulers" size="xs" />
        </div>
      ...
      <div class="pl-6 space-y-4">
        ...
        <div>
          <div class="flex items-center justify-between mb-1.5">
            ...
            <UToggle v-model="lockAspect" size="xs">
              <template #label>
