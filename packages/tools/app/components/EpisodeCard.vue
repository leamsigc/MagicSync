...
<template>
  <div
    ...
    data-testid="episode-card" @click="emit('play')">
    <div class="flex items-start gap-4">
      <div class="shrink-0 mt-1">
        <div
          class="w-10 h-10 rounded-full bg-orange-500/20 flex items-center justify-center group-hover:bg-orange-500/30 transition-colors">
          <UIcon :name="isPlaying ? 'i-lucide-pause' : 'i-lucide-play'" class="w-4 h-4 text-orange-400" />
        </div>
      </div>
      <div class="flex-1 min-w-0">
        <h4 class="text-sm font-medium  leading-snug line-clamp-2">{{ title }}</h4>
