  ...
  <div class="max-w-5xl mx-auto p-6">
    ...
    <div class="mb-6 flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <USelect
v-model="selectedFolderId" :items="folderOptions" label-key="label" value-key="id"
...
ref="fileInput" type="file" class="hidden" accept=".pdf,.txt,.md,.html,.docx,.csv,.json" multiple
      ...
      <UButton
:label="t('upload')" icon="i-heroicons-arrow-up-tray" color="primary" :loading="uploading"
        ...
        <tbody>
          <tr v-for="doc in documents" :key="doc.id" class="border-b border-neutral-100 dark:border-neutral-800">
            ...
            <td class="py-4">
              ...
              <UBadge
v-else :label="t(`status.${doc.status}`)" :color="getStatusColor(doc.status)" variant="soft"
                size="sm" />
            </td>
            <td class="py-4">
...
v-if="doc.status === 'pending' || doc.status === 'failed'"
                ...
                <UButton
v-if="ingesting[doc.id]" :label="t('actions.cancel')" icon="i-heroicons-x-mark" color="neutral"
                  variant="soft" size="xs" @click="cancelIngest(doc.id)" />
                <UButton
v-if="doc.status === 'completed'" :label="t('actions.reingest')" icon="i-heroicons-arrow-path"
